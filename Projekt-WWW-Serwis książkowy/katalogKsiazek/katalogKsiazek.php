...
<br>
  ...
  <div id="pasekMenu">
    <button class="menu"><a>KATALOG KSIĄŻEK</a></button>
        ...
        <?php
          ...
          if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                  ...
                  .$row["okladka"]. "' alt='".$row["tytul"]."' style='height: 200px; margin-left: 7px;'></a>";
                    if( $_SESSION["rola"] == 'admin') {
...
<br>
  <form id="addBook" method="post" action="addBook.php" enctype="multipart/form-data">
    ...
    <p class="text4">Gatunek:</p> <select id="gatunek" type="text" name="gatunek" placeholder="Gatunek:">
      <?php 
          ...
          if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                  echo 
                  "<option value='".$row["id"]."'>".$row["gatunek"]."</option>";
              }
          } else {
              echo "0 results";
          }
    ...
    <input id="przycisk" type="submit" name="submit" value="Dodaj nową książkę">
  </form>
<br>
  <div id="footer">
    <br>
